...
注册/登录
  ...
  → 用户轮询订单状态 / 订单列表
  → 模拟支付（余额 CAS 扣减 → 订单状态 CAS → seat_lock 置已售 → Outbox 发下游事件）
...
graph TB
    ...
    subgraph APP["Spring Boot 3 应用 (MVC 单模块)"]
        ...
        LUA --> OUTBOX["写 ORDER_CREATE Outbox<br/>立即返回 PROCESSING"]
        OUTBOX --> MQ["Relay 投递 RocketMQ"]
        MQ --> S3
        S3 --> TX["同事务: order + order_seat + seat_lock<br/>更新入口事件状态<br/>兜底: UPDATE schedule WHERE available>=n<br/>唯一索引 uk_seat"]
...
│   ├── seat_confirm.lua      # 支付成功确认 Redis 座位
│   └── token_bucket.lua
├── mq/
...
| 层 | 手段 | 作用 |
|---|---|---|
| L1 | Redis Lua：库存预扣 + 座位冲突检查/TTL 锁定 | 在入口原子拒绝冲突或库存不足请求，座位不被两人同时锁 |
...
- 支付成功走 `seat_confirm.lua`：删 reservation 凭证、把座位锁写入"已售"投影，**不返还库存**；重复支付请求由订单状态直接幂等返回。
...
| **击穿**（热点 key 过期瞬间回源） | 逻辑过期（value 带 expireTime，物理 TTL 比逻辑 TTL 晚 60~359 秒）+ 异步线程重建 + Redisson 锁 `lock:rebuild:{key}` + `MultiLevelCacheManager.localLoads` 单实例 single-flight |
| **雪崩**（大量 key 同时过期） | 物理 TTL 在业务逻辑 TTL 基础上延后 60~359 秒 + L1 Caffeine 兜底 + 接口限流 |

**读路径**：Caffeine(L1) → Redis(L2) → 布隆过滤 → DB。抢不到 Redisson 重建锁时先重读 Redis，仍为空则由 single-flight 合并本实例回源，避免每个请求各自访问 DB。
**缓存对象**：电影列表、电影详情、影院列表、场次列表；座位图实时查询 `seat_lock`，不进入 Caffeine/Redis 展示缓存。

---

## 8. 令牌桶限流
- Redis Lua 令牌桶 `token_bucket.lua`，key = `rate:{identity}:{api}`。
- 抢座接口使用 `@RateLimit(rate=5, capacity=10, granularity=USER)` + AOP 切面。
- **策略分支**：`RateLimitGranularity` 支持 USER / IP / GLOBAL，切面负责解析 identity。
- 超过令牌桶容量返回 429，保护后面的 Redis/DB。

---

## 9. Outbox 与 RocketMQ 削峰
- **Outbox**：抢座入口写入 `outbox_event` 并立即返回 `PROCESSING`；`OutboxRelayJob` 扫 `PENDING` 投递 MQ，成功标记 SENT，失败重试 10 次转 DEAD。消费者事务再写订单、order_seat、seat_lock，并更新入口事件处理状态。
...
| 模板化流程 | `MultiLevelCacheManager.get()` 固定 L1→L2→布隆→回源→回填骨架 |
| 状态机 | `SeckillStatusVO` 与订单状态控制异步落单、支付和超时关单 |
| 代理 | AOP 限流/日志/事务 |
...
| 6 | Outbox Relay + 订单超时关单补偿 + 模拟支付 `v0.6-outbox-pay` |
...
> 下表是设计阶段的验收目标，不得直接写入简历或面试回答；真实结果以 `tests/performance/压测报告.md` 为准。
| 指标 | 目标 |
|---|---|
| 抢座接口单机 QPS | ≥ 2000（Redis Lua 预扣，不等落单）|
| P99 RT（设计目标） | < 50ms |
| 超卖/座位重售 | 0（DB available 不为负，seat_lock 无重复座位）|
| 缓存综合命中率（设计目标，当前未独立采集） | > 95% |
