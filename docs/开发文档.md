...
注册/登录
  ...
  → 超时未支付：RocketMQ 定时消息 + DB 扫描双兜底关单，Lua 返还 Redis 库存/释放座位
```
...
| 消息队列 | RocketMQ | **5.x**（Docker，支持定时消息） |
| JWT | jjwt | 0.12.x |
...
├── job/
│   ├── OutboxRelayJob / OrderTimeoutJob / StockReconcileJob
└── cache/
...
| 问题 | 方案 |
|---|---|
| **穿透**（查不存在的场次/电影） | 布隆过滤器（应用启动+数据上架时加载 ID）+ 空值短 TTL 缓存（30~60s）|
...
- **削峰**：抢座瞬间 5000 QPS 打 Redis Lua 1ms 返回"排队中"，MQ 消费者按 DB 可承受速率（~500 TPS）落单，洪峰摊平。
- **订单超时**：下单时发 RocketMQ 5 定时消息（30 分钟后投递）+ DB `OrderTimeoutJob` 每分钟扫 `status=0 AND expire_time<now()` 双兜底；关单后调 `seat_release.lua` 返还 Redis。
- **库存对账**：`StockReconcileJob` 每 10 分钟对比 Redis stock 和 DB available，偏差超阈值时以 DB 为准校准（Redisson 锁保护）。
...
| 8 | 压测（wrk + JMeter）+ 压测报告 + README + 面试讲解文档 `v0.8-perf` |
