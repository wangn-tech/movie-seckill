...
```
┌────────── Frontend (Next.js 14) ──────────┐
│ 登录/注册 电影列表 电影详情 影院详情 选座    │
│ 订单列表 订单详情 模拟支付                  │
└──────────────┬──────────────────────────────┘
               │ /ajax, /api  (Knife4j: /doc.html)
┌──────────────▼──────────────────────────────┐
│         Spring Boot 3 应用 (MVC 单模块)        │
│  Controller                                   │
│   ├─ JwtAuthInterceptor → ThreadLocal UserCtx │
│   ├─ @RateLimit AOP (Redis Lua 令牌桶)        │
│   ▼                                           │
│  Service                                      │
│   ├─ MovieService/CinemaService/ScheduleSvc   │
│   │    └─ MultiLevelCache: Caffeine(L1)→Redis(L2)→DB
│   │       + 布隆过滤 + 空值缓存 + 逻辑过期+异步重建(Redisson锁)
│   ├─ SeatService (抢座核心)                   │
│   │    ① 布隆过滤器校验 scheduleId             │
│   │    ② EVAL seat_reserve.lua:              │
│   │       requestId幂等 + 库存预扣 + 座位冲突 + 锁定
│   │    ③ 成功 → 发 RocketMQ (削峰)            │
│   ├─ OrderConsumer (MQ 异步落单)              │
│   │    同事务: INSERT order + order_seat + seat_lock + outbox
│   │    兜底: UPDATE schedule SET available=available-n WHERE available>=n
│   │         seat_lock UNIQUE(schedule_id,row,col)
│   ├─ PaymentService                           │
│   │    余额扣减 + order CAS 0→1 + seat_lock 1→2
│   ├─ OutboxRelayJob (扫 PENDING 投递 MQ)      │
│   └─ OrderTimeoutJob (关单 + Lua 返还库存)    │
└──┬──────────────┬───────────────┬─────────────┘
   ▼              ▼               ▼
 MySQL 8      Redis 7         RocketMQ 5
(outbox/seat_  (Lua/布隆/      (削峰+定时消息)
 lock/order)   Caffeine远程)
```
...
### 6.1 时序
```
用户           Controller        SeatService          Redis            RocketMQ       MySQL
│ POST /seat/lock │                  │                    │                 │            │
│─────────────────►│ @RateLimit      │                    │                 │            │
│                  │────────────────►│ 布隆过滤 scheduleId │                 │            │
│                  │                 │───────────────────►│                 │            │
│                  │                 │ EVAL seat_reserve.lua:               │            │
│                  │                 │  KEYS: stock:{sid}  reservation:{rid}│
│                  │                 │        seat:{sid}:r{c}  ×N           │
│                  │                 │  ① requestId 已存在→幂等成功          │
│                  │                 │  ② 任一座位被他人锁→返回冲突          │
│                  │                 │  ③ available < seatCount→返回售罄    │
│                  │                 │  ④ DECRBY stock N                     │
│                  │                 │  ⑤ SET 各座位锁 owner=uid|rid TTL     │
│                  │                 │  ⑥ SET reservation 一次性凭证        │
│                  │                 │◄───────────────────│                 │            │
│                  │                 │ 发 MQ (不写库，立即返回)              │            │
│                  │                 │─────────────────────────────────────►│            │
│ 200 "排队中"     │◄────────────────│                    │                 │            │
